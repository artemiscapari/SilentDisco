...
filename <- "csv/localdata_"
thresholds <- c(150, 200, 250)
extension <- ".csv"

redlabel <- "red_local_cluster"
greenlabel <- "green_local_cluster"
bluelabel <- "blue_local_cluster"
...
for(i in 1:length(thresholds)) {
	...
	sumpath <- paste(barepath, "results/local_", toString(thresholds[i]), "summary.r", sep="")
	etapath <- paste(barepath, "results/local_", toString(thresholds[i]), "etasq.r", sep="")
	anopath <- paste(barepath, "results/local_", toString(thresholds[i]), "anova.r", sep="")
